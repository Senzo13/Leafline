...
interface OrganismScanProps {
  type: string;
  disease: Disease[];
}

const OrganismScan: React.FC<OrganismScanProps> = ({ type, disease }) => {
  ...
  const title =
    type === 'diseases'
      ? t('organismScan.probableDiseases')
      : t('organismScan.probablePests');
  ...
  return (
    <View
      ...
      <Accordion
        title={title}
        subtitle={t('organismScan.analysisDetails')}
        expand={false}>
        <ScanDivider />
        <Text style={{ color: theme.colors.gray }}>
          {t('organismScan.summary')}:
        </Text>
        <Text style={{ color: theme.colors.text }}>
          {t('organismScan.environmentalConditions')}
        </Text>
        <View
          style={{
            ...styles.subContent,
            ...
            if (hasHighDanger) {
              return (
                <TouchableOpacity
                  ...
                  <ImageBackground
                    source={getImageSource(type, disease.organism.name)}
                    style={styles.cardImage}
                    resizeMode="cover">
                    <View style={styles.overlay} />
                    <Text style={styles.cardText}>
                      {disease.organism.name.toUpperCase()}
                    </Text>
                  </ImageBackground>
                </TouchableOpacity>
              );
        ...
        <Modal
          ...
          <View style={styles.modalContainer}>
            <View style={styles.modalContent}>
              <ImageBackground
                source={getImageSource(type, selectedDisease.organism.name)}
                style={styles.modalImage}
                resizeMode="cover">
                <View style={styles.overlay} />
                <Text style={styles.modalTitle}>
                  {selectedDisease.organism.name.toUpperCase()}
                </Text>
              </ImageBackground>
              <Text style={styles.modalText}>
                <Text style={{ ...theme.texts.textSmallBold }}>
                  {t('organismScan.description')}:
                </Text>{' '}
                {selectedDisease.organism.description}
              </Text>
              <Text style={styles.modalText}>
                <Text style={{ ...theme.texts.textSmallBold }}>
                  {t('organismScan.probableDiseases')}:
                </Text>{' '}
                {selectedDisease.organism.probableDiseases}
              </Text>
              <Button
                title={t('organismScan.close')}
